...
class StreamServerService(server_actor.StreamServerHandler):
    ...
    async def register_stream(
        ...
        logger.info(
            "📝 Registering stream: stream_id=%s, caller=%s",
            stream_id,
            caller,
        )
        ...
        async def _send_stream_messages() -> None:
            logger.info("🚀 Starting to send stream messages for stream_id=%s", stream_id)
            ...
            for i in range(1, 6):  # Send 5 messages
                ...
                try:
                    ...
                    logger.info("📤 Sent stream %s/%s: %s", i, 5, message)
...
async def main() -> int:
    ...
    logger.info("🚀 Starting StreamRegisterServer...")
    ...
    logger.info("✅ StreamRegisterServer started! Actor ID: %s", ref.actor_id())
